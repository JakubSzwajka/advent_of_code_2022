...
mod tests {
    ...
    lazy_static! {
        static ref INPUT_TEST: usize = get_input::<Day13Pt1>("test.txt").unwrap();
        static ref INPUT_MAIN: usize = get_input::<Day13Pt1>("input.txt").unwrap();
    }

    #[test]
    fn test_part1_test() -> Result<()> {
        assert_eq!(31, Day13Pt1::solve(&INPUT_TEST)?);
        Ok(())
    }
